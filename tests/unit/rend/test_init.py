...
@pytest.mark.asyncio
async def test_rend_parse_jinja_exc(prep_hub):
    '''
    ...
    with pytest.raises(rend.exc.RenderException) as exc:
        await prep_hub.rend.init.parse(fn_, 'jinja')
    assert exc.value.args[0] == "Jinja syntax error Encountered unknown tag 'test_exc'."


@pytest.mark.asyncio
async def test_rend_parse_yaml_exc(prep_hub):
    '''
    test rend.init.parse when RendererException
...
@pytest.mark.asyncio
async def test_rend_parse_toml_exc(prep_hub):
    '''
    test rend.init.parse when RendererException
    ...
    with pytest.raises(rend.exc.RenderException) as exc:
        await prep_hub.rend.init.parse(fn_, 'toml')
